...
class FlashMessages {
	...
	//-----------------------------------------------------------------------------------------------	
	var $msgId;
	var $msgTypes = array('info', 'warning', 'success', 'danger');
	var $msgClass = 'alert';
	var $msgWrapper = "<div class='%s %s'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>\n%s</div>\n";
	var $msgBefore = '<p>';
	var $msgAfter = "</p>\n";
	...
	public function add($type, $message, $redirect_to=null) {
		...
		if( strlen(trim($type)) == 1 ) {
			$type = str_replace( array('i', 'w', 'e', 's'), array('info', 'warning', 'danger', 'success'), $type );
	...
	public function display($type='all', $print=true) {
		...
		if( in_array($type, $this->msgTypes) ) {
			...
			$data .= sprintf($this->msgWrapper, $this->msgClass, $this->msgClass.'-'.$type, $messages);
